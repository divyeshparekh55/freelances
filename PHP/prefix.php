...
session_start();
  ...
  if($_SESSION['user_type'] !== 'sub_admin') {
            ...
            if ($error === 0) {
                if ($img_size > 1250000) {
                    ...
                    if (in_array($img_ex_lc, $allowed_exs)) {
                        ...
                        $sql = "INSERT INTO prefix_images(img_nme,img_location,user_id) VALUES ('$new_img_name','$img_upload_path','$admin_id')";
        ...
        if(isset($_GET['id']))
	{
		...
		$select_id = "SELECT * FROM prefix_images where id='$user_id' ";
		...
		$query = "DELETE FROM prefix_images WHERE id= ".$fetch_recs['id'];
...
<script>
    <?php
        ...
        $users = $db->select('prefix_images',['*'],['user_id'=>$admin_id])->results();
        
    ?>

    function deleteClick(id) {
      window.location.href = 'prefix.php?id='+id;
    }
