...
session_start();
  ...
  if($_SESSION['user_type'] !== 'sub_admin') {
            ...
            if ($error === 0) {
                if ($img_size > 1250000) {
                    ...
                    if (in_array($img_ex_lc, $allowed_exs)) {
                        ...
                        $sql = "INSERT INTO postfix_images(img_nme,img_location,user_id) VALUES ('$new_img_name','$img_upload_path','$admin_id')";
        ...
        if(isset($_GET['id']))
	{
		...
		$select_id = "SELECT * FROM postfix_images where id='$user_id' ";
		...
		$query = "DELETE FROM postfix_images WHERE id= ".$fetch_recs['id'];
...
<script>
    <?php
        ...
        $users = $db->select('postfix_images',['*'],['user_id'=>$admin_id])->results();
        
    ?>

    function deleteClick(id) {
      window.location.href = 'postfix.php?id='+id;
    }
